...
#define USBD_VID           0xFFFF // 0403
#define USBD_PID           0xFFFF // 6010
...
#if 0
static uint8_t channel_io_reference_table[] = {
...
#endif
#if defined(bl702_custom) // CONFIG_GPIO15_FUNC==GPIO_FUN_ADC
adc_channel_t posChList[] = { ADC_CHANNEL1 }; // GPIO_PIN_15
#elif defined(bl702_xtzb1dk) // CONFIG_GPIO9==FUNC GPIO_FUN_ADC
adc_channel_t posChList[] = { ADC_CHANNEL7 }; // GPIO_PIN_9
#else
adc_channel_t posChList[] = { ADC_CHANNEL2 }; // GPIO_PIN_17
#endif
...
int adc_init(void)
{
    ...
    if (adc_test) {
    	...
    	ADC_DEV(adc_test)->clk_div = ADC_CLOCK_DIV_20; // ADC_CLOCK_DIV_12; // ADC_CLOCK_DIV_24;
    	ADC_DEV(adc_test)->vref = ADC_VREF_3V2;
    	ADC_DEV(adc_test)->continuous_conv_mode = ENABLE;
    	ADC_DEV(adc_test)->differential_mode = DISABLE;
        ADC_DEV(adc_test)->data_width = ADC_DATA_WIDTH_14B_WITH_16_AVERAGE; // ADC_DATA_WIDTH_16B_WITH_128_AVERAGE;
        ADC_DEV(adc_test)->fifo_threshold = ADC_FIFO_THRESHOLD_16BYTE;
    	ADC_DEV(adc_test)->gain = ADC_GAIN_NONE; //?
    ...
    if (dma_ch0) {
        ...
        DMA_DEV(dma_ch0)->transfer_mode = DMA_LLI_CYCLE_MODE; // DMA_LLI_ONCE_MODE;
        ...
        MSG("dma_ch0 device error!\r\n");
    }

    /* connect adc device and dma device */
...
void main_read_command(void)
{
    if (Ring_Buffer_Get_Length(&usb_rx_rb)) {
    	...
    	uint32_t avalibleCnt = Ring_Buffer_Read(&usb_rx_rb, buf, sizeof(buf)-1);
    	if (avalibleCnt) {
    		buf[avalibleCnt] = 0;
        	MSG("usb read: %s\r\n", buf);
        	MSG("adc_read_cnt: %s\r\n", adc_read_cnt);
        	MSG("adc_overflow_cnt: %s\r\n", adc_overflow_cnt);
    	}
#endif
    }
}
...
int main(void)
{
    ...
    uint8_t chipid[8];
    uint8_t chipid2[6];
    if(EF_Ctrl_Read_Chip_ID(chipid) == SUCCESS) {
    	bflb_platform_printf("ChipID: ");
