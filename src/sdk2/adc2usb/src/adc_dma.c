...
//    Ring_Buffer_Init(&usb_rx_rb, usb_rx_mem, USB_RX_RINGBUFFER_SIZE, _ringbuffer_lock, _ringbuffer_unlock);
    ...
    cfg.vref = ADC_VREF_3P2V; // ADC_VREF_2P0V or ADC_VREF_3P2V

    bflb_adc_init(adc, &cfg);

    // ADC GPIO_PIN_17
    chan[0].pos_chan = ADC_CHANNEL_2;
    chan[0].neg_chan = ADC_CHANNEL_GND;
    // ADC GPIO_PIN_9 (DAC OUT)
    chan[1].pos_chan = ADC_CHANNEL_7;
    chan[1].neg_chan = ADC_CHANNEL_GND;
    // diff, set cfg.differential_mode = true
    chan[2].pos_chan = ADC_CHANNEL_2; // GPIO_PIN_17
    chan[2].neg_chan = ADC_CHANNEL_7; // GPIO_PIN_9

    if(bflb_adc_channel_config(adc, &chan[1], 1) != 0)
    ...
    if(bflb_dma_channel_lli_reload(dma0_ch0, lli, 1, transfers, 1) != 0)
    	printf("ADC Error: dma channel!\r\n");

    // set DMA cycles mode
    lli[0].nextlli = (uint32_t)&lli[0];
    putreg32(lli[0].nextlli, dma0_ch0->reg_base + DMA_CxLLI_OFFSET);
